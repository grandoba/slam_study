...
def plot_track(ps, actual, zs, cov, std_scale=1,
    ...
    bp.plot_track(range(1, count + 1),actual, c='k')
    bp.plot_measurements(range(1, count + 1), zs)
    bp.plot_filter(range(1, count + 1), ps)

    plt.plot(range(1, len(std_top)+1),std_top, linestyle=':', color='k', lw=1, alpha=0.4)
    plt.plot(range(1, len(std_top)+1),std_btm, linestyle=':', color='k', lw=1, alpha=0.4)
    plt.fill_between(range(1, len(std_top)+1), std_top, std_btm,
